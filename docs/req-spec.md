...
## Things to consider while working on your solution:

- Structure, performance, and scalability considerations
- Clean and robust implementation
- The Code needs to be structured properly, separate the logic to different components . ex: controller, service , model, utils etc.
- Oolio is interested on mainly using concurrency to read the files and finding promocode. They are interested in using go-routines and channels to implement the solution. This means use separate go-routines to read each file. The file should read in streaming manner, don't load everything to memory, stream result to a channel.
- Read from channel, and whenever the result is found i.e., promocode found in two file locations, cancel the running go-routines and return the result.
